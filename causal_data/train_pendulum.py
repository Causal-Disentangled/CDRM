...
from pendulum_data import load_data
from gnn_pendulum import train_gnn_pendulum
from scm_pendulum import dag
from restore_pendulum import restore
...
def main():
    ...
    parser.add_argument('--log_dir', type=str, default='31')
    ...
    if not os.path.exists('./causal_data/pendulum/'):
        os.makedirs('./causal_data/pendulum/')
    log_path = './causal_data/pendulum/{}_{}_{}/'.format(args.log_dir, args.ratio, args.epochs)
    os.makedirs(log_path)
    train_gnn_pendulum(data, args, log_path, device)
    dag(args)
    restore(args)
