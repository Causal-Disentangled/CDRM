...
class CausalVAE(nn.Module):
    def __init__(self, nn='mask', name='vae', z_dim=16, z1_dim=4, z2_dim=4, z3_dim=3,inference = False):
        super().__init__()
        self.name = name
        self.z_dim = z_dim
        self.z1_dim = z1_dim
    ...
    def negative_elbo_bound(self, x, label, dag, mask = None, sample = False, adj = None, lambdav=0.001):
        """
        ...
        decode_m, decode_v = self.dag.calculate_dag(q_m.to(device), torch.ones(q_m.size()[0], self.z1_dim,self.z2_dim).to(device), dag.to(device))
        ...
        if sample == False:
          ...
          m_zm, m_zv = self.dag.mask_z(decode_m.to(device), dag.to(device)).reshape([q_m.size()[0], self.z1_dim,self.z2_dim]),decode_v.reshape([q_m.size()[0], self.z1_dim,self.z2_dim])
        ...
        for i in range(self.z1_dim):
            kl = kl+ut.kl_normal(decode_m[:,i,:].to(device), cp_v[:,i,:].to(device),cp_m[:,i,:].to(device), cp_v[:,i,:].to(device))
        kl = torch.mean(kl)
        mask_kl = torch.zeros(1).to(device)


        for i in range(self.z1_dim):
            mask_kl = mask_kl+ut.kl_normal(f_z1[:,i,:].to(device), cp_v[:,i,:].to(device),cp_m[:,i,:].to(device), cp_v[:,i,:].to(device))

        mask_l = torch.mean(mask_kl)
        nelbo = rec + kl + mask_l
