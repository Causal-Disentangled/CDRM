...
class DagLayer(nn.Linear):
	...
	def mask_z(self, x, dag):
		...
		#    return x
		x = torch.matmul(dag.t(), x)

		return x

	def mask_u(self, x, dag):
		self.B = self.A
		# if self.i:
		#    x = x.view(-1, x.size()[1], 1)
		#    x = torch.matmul((self.B+0.5).t().int().float(), x)
	...
	def calculate_dag(self, x, v, dag):
		if x.dim() > 2:
			x = x.permute(0, 2, 1)
		#dag = dag.to(torch.float32)
		x = F.linear(x, torch.cholesky_inverse(self.I - dag.t()), self.bias)
