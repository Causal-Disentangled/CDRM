...
def condition_prior(scale, label, dim):
    mean = torch.ones(label.size()[0], label.size()[1], dim)  # 64 4 4
    var = torch.ones(label.size()[0], label.size()[1], dim)  # 64 4 4
    for i in range(label.size()[0]):  # 64
        for j in range(label.size()[1]):  # 4
