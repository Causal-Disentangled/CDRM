...
if args.dag == "sup_dag":
  ...
  ut.load_model_by_name(lvae, 100)#迭代0次
...
def load_dag():
	...
	df_np = np.loadtxt(flow_path+'/causal_data/causal_data/flow/12/dag.txt')
	return df_np
dag = load_dag()
dataset_dir = './causal_data/causal_data/flow/12'
...
for u,l in train_dataset:
  for i in range(4):
    for j in range(-5,5):
      L, kl, rec, reconstructed_image,_= lvae.negative_elbo_bound(u.to(device),l.to(device), dag, i, adj=j*0)
    save_image(reconstructed_image, 'figs_test_vae_flow/reconstructed_image_{}_{}.png'.format(i, count),  range = (0,1))
  save_image(u, './figs_test_vae_flow/true_{}.png'.format(count))
  count += 1
  if count == 10:
    break
