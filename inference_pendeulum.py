...
dataset_dir = './causal_data/causal_data/pendulum/7'
...
for u,l in train_dataset:
  for i in range(4):
    for j in range(-5,5):
      L, kl, rec, reconstructed_image,_= lvae.negative_elbo_bound(u.to(device),l.to(device), dag, i,sample = sample, adj=j*0)
    save_image(reconstructed_image, 'figs_test_vae_pendulum/reconstructed_image_{}_{}.png'.format(i, count),  range = (0,1))
  save_image(u, './figs_test_vae_pendulum/true_{}.png'.format(count))
  count += 1
  if count == 10:
    break
