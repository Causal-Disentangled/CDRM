...
parser.add_argument('--epoch_max',   type=int, default=11,    help="Number of training epochs")
...
if not os.path.exists('./figs_vae_pendulum/'):
	os.makedirs('./figs_vae_pendulum/')
def load_dag():
	flow_path = osp.dirname(osp.abspath(inspect.getfile(inspect.currentframe())))
	df_np = np.loadtxt(flow_path+'/causal_data/causal_data/pendulum/1_0.8_3000/dag.txt')
	return df_np
dag = load_dag()
dataset_dir = './causal_data/causal_data/pendulum/1_0.8_3000'
train_dataset = get_batch_unin_dataset_withlabel(dataset_dir, 8)
...
for epoch in range(args.epoch_max):
	...
	for u, l in train_dataset:
		...
		total_rec += rec.item()
		m = len(train_dataset)
		save_image(u, 'figs_vae_pendulum/reconstructed_image_true_{}.png'.format(epoch), normalize=True)
		save_image(reconstructed_image, 'figs_vae_pendulum/reconstructed_image_{}.png'.format(epoch), range=(0, 1), normalize=True)
	if epoch % 1 == 0:
