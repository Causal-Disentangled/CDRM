...
class dataload_withlabel(data.Dataset):
    def __init__(self, root, dataset="train"):
        root = root + "/" + dataset
        imgs = os.listdir(root)
        #imgs = [list(map(int,k[:-4].split("_")[5:]))  for k in imgs]
        imgs.sort(key=lambda x: int(x.split('_')[5][:-4]))
        #print(imgs)
        #imgs.sort(key=lambda x: (int(np.array(x[:-4].split("_")))))
        #print(imgs)
        self.dataset = dataset
        self.imgs = [os.path.join(root, k) for k in imgs]
        self.imglabel = [list(map(int,k[:-4].split("_")[1:-1]))  for k in imgs]
        self.transforms = transforms.Compose([transforms.ToTensor()])
